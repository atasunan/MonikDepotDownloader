...
class SteamUploader(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('MonikDepot Downloader')
        ...
        title = QLabel('MonikDepot Downloader')
        ...
        ''')
        discord_btn.clicked.connect(lambda: QDesktopServices.openUrl(QUrl('https://dsc.gg/mtamc')))
        ...
        ''')
        github_btn.clicked.connect(lambda: QDesktopServices.openUrl(QUrl('https://github.com/atasunan/MonikDepotDownloader')))
                ...
                for file in zip_ref.namelist():
                    if file.endswith('.lua'):
                        ...
                        with zip_ref.open(file) as src, open(target, 'wb') as dst:
                            shutil.copyfileobj(src, dst)
                        manifest_count += 1
            marcellus_path = os.path.join(stplugin_dir, 'monik.lua')
            existing_lines = []
